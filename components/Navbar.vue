  ...
  <nav>
    <v-app-bar height="100px" app>
      ...
      <!--        <span>{{ // this.username}}</span>-->
        <span>Форма Отчетов</span>
      </v-btn>

      <v-spacer></v-spacer>
      <v-btn title='Пользователь' text color='white' class="custom-button" style="color: white">
        <v-icon left>mdi-account</v-icon>
<!--        <span>{{ // this.username}}</span>-->
        <span>User</span>
      </v-btn>
      <v-btn key="logout" title='Выйти из аккаунта' text color='white' @click="logout" style="color: white">
        <span>Выйти</span>
      </v-btn>
    </v-app-bar>
